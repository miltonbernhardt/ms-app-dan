...
public class PedidoServiceImpl implements PedidoService {
	...
	public Pedido crearPedido(Pedido p) throws RiesgoException {
		...
		Double totalOrden = p.getDetalle().stream().mapToDouble(dp -> dp.getCantidad() * dp.getPrecio()).sum();
	...
	@Override
	public Pedido actualizarEstadoPedido(Integer idPedido, EstadoPedido nuevoEstadoPedido)
		...
		if (pedido.isPresent()) {
			...
			if (nuevoEstadoPedido == EstadoPedido.CONFIRMADO) {
				 ...
				 */
				boolean hayStock = p.getDetalle().stream()
				 ...
				 */
				if (e == EstadoPedido.NUEVO || e == EstadoPedido.CONFIRMADO || e == EstadoPedido.PENDIENTE) {
			...
			throw new RecursoNoEncontradoException("No se encontro el pedido: ", idPedido);
		return null;
	}
	...
	public ArrayList<Pedido> pedidosPorCliente(Integer idCliente) throws RecursoNoEncontradoException {
		...
		for (Obra o : obras)
			pedidos.addAll(pedidoRepository.getPedidosPorObra(o.getId()));
		return pedidos;
	}
